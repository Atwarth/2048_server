...
# empty the next cell
                mat[i][j] = mat[i][j] * 2
                mat[i][j + 1] = 0
                #reward+= mat[i][j]# make bool variable True indicating# the new grid after merging is# different.
                changed = True
                reward+=mat[i][j]

    return mat, changed, reward
...
def move_left(grid):
	...
	new_grid, changed2, reward = merge(new_grid)
	...
	# or different
	return new_grid, changed, reward
...
def move_right(grid):
	...
	# then move left
	new_grid, changed, reward = move_left(new_grid)
...
def move_down(grid):
	...
	new_grid, changed, reward = move_right(new_grid)
